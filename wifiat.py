...
class at(object):
    ...
    def http(self,url,method="GET",ua=1,content=1,data='',timeout=6):
        ...
        # 构造请求   
        c="%s %s HTTP/1.1\r\nHOST:%s\r\nContent-Type:%s\r\nUser-Agent:%s\r\n %s \r\n\r\n " % (method,"/"+rel,domain,content,ua,data)
    ...
    def get_weather(self,location):
        method = 'GET'
        url = 'http://api.seniverse.com/v3/weather/now.json?key=SQb5Q0jZHGTFG8nq9&location={}&language=en&unit=c'.format(location)
        ...
        str = self.sendCMD_waitRespLine(c,1000*3)
